from numpy import dot, random, array, append, round, average
...
class Perceptron:
    ...
    def fit_batch(self, X, y):
        ...
        y_diff = []
        for x, y_ in zip(X, y):
            y_diff.append((y_ - dot(x, self.weights)) * x)
        self.weights = self.weights + self.learning_rate * sum(y_diff) / len(X)
